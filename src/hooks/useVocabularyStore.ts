...
export function useVocabularyStore() {
  ...
  const [subscriptionStatus, setSubscriptionStatus] = useState<{ subscribed: boolean; subscription_tier?: string } | null>(null);
  const { user } = useAuth();
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Combine lists with premium filtering
      const combinedLists: VocabularyList[] = [
        ...
        ...defaultLists.map(list => {
          ...
          const isPremium = (list as any).premium_required === true;
          const canActivate = !isPremium || subscriptionStatus?.subscribed === true;
          
          return {
            id: list.id,
            name: list.name,
            isActive: canActivate ? (preference?.is_active ?? true) : false,
      ...
      // Load vocabularies for active lists only
      const activeListIds = combinedLists
      ...
      const [userVocabsResult, defaultVocabsResult] = await Promise.all([
        ...
        // Load default vocabularies for active lists
        activeListIds.length > 0 
          ? supabase
              .from('default_vocabularies')
              .select('*')
              .in('list_id', activeListIds.filter(id => combinedLists.find(l => l.id === id)?.isDefault))
      ...
      const mappedDefaultVocabs: Vocabulary[] = defaultVocabs.map(vocab => ({
        ...
        german: vocab.german,
        listId: vocab.list_id,
        box: 0, // Default vocabs start at box 0
  ...
  const moveVocabularyToBox = async (vocabId: string, newBox: number, isCorrect: boolean) => {
    ...
    try {
      if (vocab.isDefaultVocab) {
        // For default vocabularies: would need user_vocabulary_progress table
        console.log('Default vocab progress tracking not implemented yet');
      } else {
        // For user vocabularies: update in vocabularies table
  ...
  const toggleVocabularyList = async (listId: string, isActive: boolean) => {
    ...
    // Check if this is a premium list and user doesn't have subscription
    const list = lists.find(l => l.id === listId);
    if (list?.isPremium && !subscriptionStatus?.subscribed && isActive) {
    ...
    try {
      ...
      // Reload vocabularies to reflect the change
      await loadData();
    } catch (error) {
